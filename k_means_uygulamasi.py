...
def k_means(dataset):
    while (True):
        ...
        for i in range(k):
            ...
            for j in range(len(dataset)):
                if dataset.iloc[j, 2] == i:
                    birinco_kolon.append(dataset.iloc[j, 0]) # iloc[satır, sütun]
        ...
        birinci_kolon_listesi = merkezler.iloc[:, 0].tolist()
        ikinci_kolon_listesi = merkezler.iloc[:, 1].tolist()

        if yeni_birinci_kolon == birinci_kolon_listesi and yeni_ikinci_kolon == ikinci_kolon_listesi:
            break
        else:
